...
async function listProjectsWithAccessFilter(params: {
  ...
  let query = params.extended
    ? supabase
      .from("projects")
      .select("id, user_id, nome, descricao, status, tags, contexto, objetivo, stakeholders, maturidade, ativo, created_at")
...
async function createProjectLegacy(input: CreateProjectInput): Promise<ProjectRecord | null> {
  ...
  if (inserted.error || !inserted.data) return null;
  return mapProjectRow(inserted.data as ProjectRowBase);
}
...
export async function updateProject(params: {
  ...
  projectId: string;
  patch: Partial<
    Pick<ProjectRecord, "name" | "description" | "tags" | "context" | "objective" | "stakeholders" | "maturity" | "status">
  >;
}): Promise<ProjectRecord | null> {
